...
try:
    ...
    for inLayer in llist:
        ...
        outLayer = arcpy.ValidateTableName(outLayer)
        ...
        print "Fetching maximum data sizes..."
        sys.stdout.flush()           
        
        # get max data lengths for all string fields        
        for row in arcpy.da.SearchCursor(inLayer, fnList):
            for fdx in xrange(0, len(fnList)):
                if row[fdx] is None:
                    clen = 0
                else:
                    clen = len(row[fdx])
                if clen > fsMap[fnList[fdx]]:
                    fsMap[fnList[fdx]] = clen
        ...
        for f in fieldList:
            ...
            if of.type == "String":
                if of.length > fsMap[f.name]:
                    of.length = fsMap[f.name]
